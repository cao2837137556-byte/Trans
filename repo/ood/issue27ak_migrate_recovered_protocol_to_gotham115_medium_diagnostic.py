...
ISSUE = "issue27ak_migrate_recovered_protocol_to_gotham115_medium_diagnostic_2026-06-02"
OUT = ROOT / "runs" / ISSUE
...
ISSUE27AI = ROOT / "runs" / "issue27ai_medium_protocol_audit_then_diagnostic_2026-06-02"
ISSUE27AJ = ROOT / "runs" / "issue27aj_protocol_lineage_recovery_and_support_selector_audit_2026-06-02"
...
def first_support_indices(sidecar: list[dict[str, str]], support_size: int = SUPPORT_SIZE) -> np.ndarray:
    return np.flatnonzero(role_mask(sidecar, SUPPORT_ROLE))[:support_size]
...
def fixed_support_mask(sidecar: list[dict[str, str]], support_size: int = SUPPORT_SIZE) -> np.ndarray:
    ...
    mask[first_support_indices(sidecar, support_size)] = True
    return mask
...
def kcenter_support_indices(x: np.ndarray, sidecar: list[dict[str, str]], support_size: int = SUPPORT_SIZE) -> tuple[np.ndarray, dict[str, Any]]:
    pool_idx = np.flatnonzero(role_mask(sidecar, SUPPORT_ROLE))
    if len(pool_idx) == 0:
        raise RuntimeError("empty attack_support pool")
    x_pool = x[pool_idx]
    ...
    local_idx = farthest_first_indices(z, support_size, start_idx)
    selected = np.asarray(sorted(pool_idx[local_idx]), dtype=np.int64)
    audit = {
        ...
        "deterministic": True,
        "selected_global_row_sha256": hashlib.sha256(",".join(map(str, selected.tolist())).encode("utf-8")).hexdigest(),
    }
...
MATRIX = [
    MatrixRow("LR", "lr", "raw_support_id_threshold", "id_support32", "id_q99", True, False, False, "raw supervised head with kcenter32 support and ID threshold"),
    MatrixRow("LR", "lr", "threshold_guard_only", "id_support32", "ood_val_q99", True, True, True, "same LR score, OOD-val threshold pressure"),
    MatrixRow("LR", "lr", "full_guarded_protocol", "id_support32", "max_id_ood_val_q99", True, True, True, "LR score with kcenter32 support and ID/OOD safety"),
    ...
    MatrixRow("HistGB", "histgb", "raw_support_id_threshold", "id_support32", "id_q99", True, False, False, "raw HistGB with kcenter32 support and ID threshold"),
    ...
    MatrixRow("HistGB", "histgb", "full_guarded_protocol", "id_support32", "max_id_ood_val_q99", True, True, True, "HistGB score with kcenter32 support and ID/OOD safety"),
    ...
    MatrixRow("DeepSADStyle_Lite", "deepsad", "raw_support_id_threshold", "id_support32", "id_q99", True, False, False, "center-style score with kcenter32 support and ID threshold"),
    ...
    MatrixRow("DevNetStyle_MLP", "mlp", "raw_support_id_threshold", "id_support32", "id_q99", True, False, False, "small MLP score head with kcenter32 support and ID threshold"),
    ...
    MatrixRow("DevNetStyle_MLP", "mlp", "full_guarded_protocol", "id_support32", "max_id_ood_val_q99", True, True, True, "MLP score with kcenter32 support and ID/OOD safety"),
...
def fit_mask_and_labels(sidecar: list[dict[str, str]], fit_mode: str, support_mask: np.ndarray) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    if fit_mode == "id_support32":
        ...
        return mask, y, f"{ID_ROLE}|{SUPPORT_ROLE}:{SUPPORT_SELECTOR}_{SUPPORT_SIZE}"
...
def main() -> None:
    ...
    aj_summary = (ISSUE27AJ / "summary.md").read_text(encoding="utf-8")

    audit_rows: list[dict[str, Any]] = []
    matrix_rows: list[dict[str, Any]] = []
    support_rows: list[dict[str, Any]] = []
    support_index_rows: list[dict[str, Any]] = []
    support_compare_rows: list[dict[str, Any]] = []
    ...
    first32_delta_rows: list[dict[str, Any]] = []
    ...
    audit_rows.append({"check": "issue27aj_recovered_kcenter_lineage", "ok": "recovered_kcenter_mainline_protocol_ready_for_gotham115_migration" in aj_summary, "detail": "issue27aj protocol lineage gate"})
    ...
    for strategy in sorted(cert):
        ...
        kcenter_idx, selector_audit = kcenter_support_indices(x, sidecar, SUPPORT_SIZE)
        support_mask = mask_from_indices(len(sidecar), kcenter_idx)
        first_idx = first_support_indices(sidecar, SUPPORT_SIZE)
        first_set = set(map(int, first_idx.tolist()))
        kcenter_set = set(map(int, kcenter_idx.tolist()))
        attack_mask = role_mask(sidecar, ATTACK_EVAL_ROLE)
        support_rows.append(
            {
                "strategy": strategy,
                "support_rule": f"{SUPPORT_SELECTOR}_from_preregistered_attack_support_role",
        ...
        for rank, idx in enumerate(kcenter_idx.tolist()):
            support_index_rows.append(
                {
                    "strategy": strategy,
                    "selector_name": SUPPORT_SELECTOR,
                    "rank_after_sorted_output": rank,
                    "global_row_index": int(idx),
                    "role": sidecar[int(idx)].get("role", ""),
                    "label": sidecar[int(idx)].get("label", ""),
                    "model_ready_hint": sidecar[int(idx)].get("model_ready_hint", ""),
                }
            )
        support_compare_rows.append(
            {
                "strategy": strategy,
                "kcenter_size": int(len(kcenter_set)),
                "first32_size": int(len(first_set)),
                "overlap_count": int(len(kcenter_set & first_set)),
                "kcenter_only_count": int(len(kcenter_set - first_set)),
                "first32_only_count": int(len(first_set - kcenter_set)),
                "comparison_scope": "static_selector_overlap_only_no_performance_conclusion",
            }
        )
        checks = [
            ...
            ("kcenter_support_available", support_mask_all.sum() >= SUPPORT_SIZE and support_mask.sum() == SUPPORT_SIZE, f"available={support_mask_all.sum()} selected={support_mask.sum()} needed={SUPPORT_SIZE}"),
        ...
        for row in MATRIX:
            ...
            if inc:
                ...
                continue
            fit_mask, y_fit, fit_roles = fit_mask_and_labels(sidecar, row.fit_mode, support_mask)
            access_rows.append(
                {
                    "strategy": strategy,
                    "head_name": row.head_name,
    ...
    if forbidden_blocked:
        verdict = "recovered_kcenter32_medium_blocked_by_forbidden_role_access"
    elif not audit_pass:
        verdict = "recovered_kcenter32_medium_blocked_before_diagnostic"
    elif not any_pass:
        verdict = "recovered_kcenter32_medium_blocked_by_adapter_failures"
    elif any_interface_failed:
        verdict = "recovered_kcenter32_medium_diagnostic_partial_interface_issue"
    else:
        verdict = "recovered_kcenter32_medium_diagnostic_completed"

    previous_path = ISSUE27AI / "medium_protocol_diagnostic_by_strategy.csv"
    if previous_path.exists():
        first32_rows = load_csv(previous_path)
        kcenter_pass = [r for r in diagnostic_rows if r.get("status") == "pass"]
        first_map = {
            (r.get("strategy"), r.get("head_name"), r.get("variant_name")): r
            for r in first32_rows
            if r.get("status") == "pass"
        }
        for r in kcenter_pass:
            key = (r.get("strategy"), r.get("head_name"), r.get("variant_name"))
            old = first_map.get(key)
            if not old:
                continue
            old_det = maybe_float(old.get("attack_eval_detection"))
            new_det = maybe_float(r.get("attack_eval_detection"))
            old_ood = maybe_float(old.get("final_ood_alarm"))
            new_ood = maybe_float(r.get("final_ood_alarm"))
            first32_delta_rows.append(
                {
                    "strategy": r.get("strategy"),
                    "head_name": r.get("head_name"),
                    "variant_name": r.get("variant_name"),
                    "first32_attack_eval_detection": old_det,
                    "kcenter32_attack_eval_detection": new_det,
                    "delta_attack_eval_detection": None if old_det is None or new_det is None else new_det - old_det,
                    "first32_final_ood_alarm": old_ood,
                    "kcenter32_final_ood_alarm": new_ood,
                    "delta_final_ood_alarm": None if old_ood is None or new_ood is None else new_ood - old_ood,
                    "comparison_scope": "diagnostic_only_no_model_ranking",
                }
            )
    ...
    write_csv(OUT / "kcenter32_support_selector_audit.csv", support_rows)
    write_csv(OUT / "kcenter32_selected_support_indices.csv", support_index_rows)
    write_csv(OUT / "support_selector_static_comparison.csv", support_compare_rows)
    ...
    write_csv(OUT / "first32_vs_kcenter32_medium_diagnostic_delta.csv", first32_delta_rows)
    ...
    write_md(
        ...
        [
            ...
            f"- support rule: recovered `{SUPPORT_SELECTOR}` with budget `{SUPPORT_SIZE}` from preregistered attack_support role.",
    ...
    write_md(
        ...
        [
            ...
            f"- Support selector: recovered `{SUPPORT_SELECTOR}`, not issue27ai fixed_first32.",
            "- `first32_vs_kcenter32_medium_diagnostic_delta.csv` is a diagnostic delta table only; it is not model ranking.",
    ...
    def find_diag(strategy: str, head: str, variant: str) -> dict[str, Any] | None:
        for r in diagnostic_rows:
            if r.get("strategy") == strategy and r.get("head_name") == head and r.get("variant_name") == variant and r.get("status") == "pass":
                return r
        return None

    def fmt_diag(r: dict[str, Any] | None) -> str:
        if not r:
            return "not available"
        return (
            f"final_ood_alarm={float(r['final_ood_alarm']):.6f}, "
            f"attack_eval_detection={float(r['attack_eval_detection']):.6f}, "
            f"feasible_under_1pct={r['feasible_under_1pct']}"
        )

    first_support_audit = support_rows[0] if support_rows else {}
    key_lines = [
        "# Diagnostic Key Findings",
        "",
        "- Scope: medium Gotham Kitsune115 diagnostic only; not a formal benchmark and not a model ranking.",
        f"- Support selector: `{SUPPORT_SELECTOR}`; support pool rows = {first_support_audit.get('support_pool_rows', 'unknown')}; selected rows = {first_support_audit.get('support_size_selected', 'unknown')}.",
        ...
        "",
        f"- LR full guarded: {fmt_diag(find_diag('reset_at_split_boundary', 'LR', 'full_guarded_protocol'))}.",
        f"- HistGB full guarded: {fmt_diag(find_diag('reset_at_split_boundary', 'HistGB', 'full_guarded_protocol'))}.",
        f"- DeepSADStyle_Lite full guarded: {fmt_diag(find_diag('reset_at_split_boundary', 'DeepSADStyle_Lite', 'full_guarded_protocol'))}.",
        f"- DevNetStyle_MLP full guarded: {fmt_diag(find_diag('reset_at_split_boundary', 'DevNetStyle_MLP', 'full_guarded_protocol'))}.",
        "",
        "## Train State Then Eval Online",
        "",
        f"- LR full guarded: {fmt_diag(find_diag('train_state_then_eval_online', 'LR', 'full_guarded_protocol'))}.",
        f"- HistGB full guarded: {fmt_diag(find_diag('train_state_then_eval_online', 'HistGB', 'full_guarded_protocol'))}.",
        f"- DeepSADStyle_Lite full guarded: {fmt_diag(find_diag('train_state_then_eval_online', 'DeepSADStyle_Lite', 'full_guarded_protocol'))}.",
        f"- DevNetStyle_MLP full guarded: {fmt_diag(find_diag('train_state_then_eval_online', 'DevNetStyle_MLP', 'full_guarded_protocol'))}.",
        "",
    ...
    write_md(OUT / "diagnostic_key_findings.md", key_lines)

    write_md(
        OUT / "issue27ak_decision.md",
        [
            "# issue27ak Decision",
            "",
            f"primary_verdict = `{verdict}`",
            "",
            "A audit passed before B diagnostic unless otherwise noted. This issue migrates the recovered kcenter32 selector to Gotham115 medium diagnostics. It may guide protocol debugging, but cannot determine the final model or paper claim.",
        ],
    )
    write_md(
        OUT / "claim_update_after_issue27ak.md",
        [
            "# Claim Update After issue27ak",
            "",
            "- Medium Gotham Kitsune115 diagnostics can be used to rehearse protocol mechanics.",
            "- issue27ak replaces the issue27ai fixed_first32 placeholder with the recovered kcenter32 selector.",
    ...
    write_md(
        OUT / "issue27al_next_action.md",
        [
            "# issue27al Next Action",
            "",
            "Recommended next issue: `issue27al_full_contract_materialization_or_exclusion_policy_and_protocol_freeze_2026-06-02`. It should either materialize the heavy ip-camera full_contract with a faster frontend/Slurm path or freeze a defensible exclusion policy, then lock support selector/support size, threshold rule, model configs, and state strategy before formal benchmark.",
        ],
    )
    config = {
        "issue": ISSUE,
        "task_type": "migrate_recovered_kcenter32_to_gotham115_medium_diagnostic",
        "formal_benchmark": False,
        "model_ranking": False,
    ...
    write_md(
        ...
        [
            ...
            "6. model ranking: no.",
            f"7. support selector: `{SUPPORT_SELECTOR}`.",
            f"8. support size: `{SUPPORT_SIZE}`.",
            f"9. forbidden role access blocked: `{forbidden_blocked}`.",
            f"10. protocol matrix rows: `{len(MATRIX)}`.",
            "11. final eval use: report-only; no fit/threshold/selection.",
            "12. full_contract_pending: `True`.",
            "13. next recommendation: full_contract/exclusion policy plus protocol freeze before formal benchmark.",
