...
V4B_CHANNEL_NAMES = [
    "mean_rel_slog", "std_rel_slog", "dispersion_slog", "number_centered_log",
    "cov_relative_slog", "pcc_centered_slog", "short_long_mean_log_ratio", "short_long_dispersion_log_ratio",
]
...
def load_v4b_matrix(path: Path) -> np.ndarray:
    """Load expression_v4b_hh_soft_stabilized_matrix [N,20,8]."""
    arr = np.load(path)
...
def plot_tradeoff(df: pd.DataFrame, out: Path) -> None:
    ...
    plt.title("Frontend-F2 expression_v4b fixed trade-off")
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Frontend-F2 expression_v4b tokenizer v1.")
    ap.add_argument("--run-tag", default=f"frontend_f2_expression_v4b_tokenizer_v1_{today}")
    ap.add_argument(
        "--benign-data-dir", type=Path,
        default=WORKTREE_ROOT / "runs" / "frontend_f2_expression_v4b_crosscapture_stage1_2026-04-20" / "data",
        help="Directory containing id_source_expression_v4b_hh_soft_stabilized_matrix.npy and ood_benign_source_expression_v4b_hh_soft_stabilized_matrix.npy",
    )
    ap.add_argument(
        "--attack-data-dir", type=Path,
        default=WORKTREE_ROOT / "runs" / "frontend_f2_expression_v4b_attack_source_2026-04-20" / "data",
        help="Directory containing attack_source_expression_v4b_hh_soft_stabilized_matrix.npy",
    )
    ap.add_argument(
    ...
    plot_dir = out / "frontend_f2_expression_v4b_plots"
    plot_dir.mkdir(exist_ok=True)
    (out / "command.txt").write_text("python " + " ".join(sys.argv) + "\n", encoding="utf-8")

    # --- load data ---
    id_matrix  = load_v4b_matrix(args.benign_data_dir / "id_source_expression_v4b_hh_soft_stabilized_matrix.npy")
    ood_matrix = load_v4b_matrix(args.benign_data_dir / "ood_benign_source_expression_v4b_hh_soft_stabilized_matrix.npy")
    atk_matrix = load_v4b_matrix(args.attack_data_dir  / "attack_source_expression_v4b_hh_soft_stabilized_matrix.npy")
    ...
    model_builders = [
        (
            "frontend_f2_expression_v4b_transformer_v1",
        ...
        (
            "frontend_f2_expression_v4b_token_mlp_control",
    ...
    score_modes = [
        ("token_mean",          "mean RMSE over all 20 expression_v4b tokens"),
        ("short_scale_mean",    "mean RMSE over scale 0.1/0.01 tokens"),
        ("weighted_token_mean", "family/scale weighted RMSE over all 20 expression_v4b tokens"),
        ...
        for detector_family, builder in model_builders:
            ...
            for mode, mode_desc in score_modes:
                ...
                extra_meta = {
                    ...
                    "input_tensor":    "expression_v4b_hh_soft_stabilized_matrix[20x8]",
                    "channel_names":   ",".join(V4B_CHANNEL_NAMES),
    ...
    df.to_csv(out / "frontend_f2_expression_v4b_results.csv",      index=False)
    df.to_csv(out / "results.csv",                                 index=False)
    diag.to_csv(out / "frontend_f2_expression_v4b_diagnostics.csv", index=False)
    plot_tradeoff(df, plot_dir / "fixed_tradeoff_frontend_f2_expression_v4b.png")

    # ── 合并表：fixed + id_budget_calibrated ──────────────────────────────
    combined = make_combined_summary(df, calib_policy=ID_CALIB_POLICY)
    combined.to_csv(out / "frontend_f2_expression_v4b_combined.csv", index=False)
    ...
    summary = "\n".join(
        [
            "# Frontend-F2 Expression v4b Tokenizer v1",
            "",
            "- Data: Frontend-F2 expression_v4b_hh_soft_stabilized sources (`7-6` ID, `4-1` OOD, `34-1` attack).",
            "- Input tensor: `expression_v4b_hh_soft_stabilized_matrix [20,8]` with dual-branch embedding "
            "(per-token 6ch + cross-scale 2ch) + family/scale positional.",
            f"- expression_v4b channels: `{', '.join(V4B_CHANNEL_NAMES)}`.",
    ...
    cfg = {
        "stage":        "frontend_f2_expression_v4b_tokenizer_v1",
        ...
        "outputs": {
            "results":     str(out / "frontend_f2_expression_v4b_results.csv"),
            "combined":    str(out / "frontend_f2_expression_v4b_combined.csv"),
            "diagnostics": str(out / "frontend_f2_expression_v4b_diagnostics.csv"),
            "summary":     str(out / "summary.md"),
            "plots":       str(plot_dir),
        },
    }
    (out / "frontend_f2_expression_v4b_manifest.json").write_text(
        json.dumps(cfg, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (out / "config.json").write_text(json.dumps(cfg, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"[done] frontend-f2 expression_v4b tokenizer output: {out}", flush=True)
