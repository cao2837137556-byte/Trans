...
def aggregate_token_scores(per_scale: np.ndarray, mode: str) -> np.ndarray:
...
def plot_tradeoff(df: pd.DataFrame, out: Path) -> None:
    ...
    colors = {
        "timescale_transformer_v1_1": "#1f77b4",
    ...
    plt.title("Timescale tokenizer v1.1 fixed trade-off")
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def append_map(run_tag: str) -> None:
    ...
    if f"`{run_tag}`" in text:
        return
    entry = f"\n- `{run_tag}`: Frontend100 TimescaleTokenizer-v1.1 single-seed scoring refinement with header-aware 5x20 regrouping and short-scale aware aggregations; path: `runs/{run_tag}/`.\n"
    p.write_text(text.rstrip() + entry, encoding="utf-8")


def main() -> None:
    today = datetime.now().strftime("%Y-%m-%d")
    ap = argparse.ArgumentParser(description="Header-aware timescale tokenizer v1.1 for frontend100.")
    ap.add_argument("--run-tag", default=f"frontend100_timescale_tokenizer_v1_1_{today}")
    ...
    plot_dir = out / "timescale_tokenizer_v1_1_plots"
    ...
    score_modes = [
        ("mean_all", "mean over all 5 scales"),
        ("max_all", "max over all 5 scales"),
        ("short_mean", "mean over short scales 0.1/0.01"),
        ("short_max", "max over short scales 0.1/0.01"),
    ...
    for mode, mode_desc in score_modes:
        ...
        sid = aggregate_token_scores(tfm_id_scales, mode)
        sood = aggregate_token_scores(tfm_ood_scales, mode)
        satt = aggregate_token_scores(tfm_attack_scales, mode)
        score_bank[obj] = (sid, sood[args.calibration_budget :], satt[high])
        rows.extend(
            eval_scores(
                object_label=obj,
                detector_family="timescale_transformer_v1_1",
                score_label=mode,
                seed=args.seed,
                score_id=sid,
                score_ood=sood,
    ...
    diagnostics.append(
        {
            "object_label": "timescale_transformer_v1_1",
    ...
    for mode, mode_desc in score_modes:
        ...
        sid = aggregate_token_scores(mlp_id_scales, mode)
        sood = aggregate_token_scores(mlp_ood_scales, mode)
        satt = aggregate_token_scores(mlp_attack_scales, mode)
        score_bank[obj] = (sid, sood[args.calibration_budget :], satt[high])
        rows.extend(
            eval_scores(
                object_label=obj,
    ...
    diagnostics.append(
        {
            "object_label": "timescale_token_mlp_v1_1",
    ...
    df.to_csv(out / "timescale_tokenizer_v1_1_results.csv", index=False)
    df.to_csv(out / "results.csv", index=False)
    diag.to_csv(out / "timescale_tokenizer_v1_1_diagnostics.csv", index=False)

    fixed = df[(df["policy_name"].eq("fixed_id_q99")) & (df["selection_feasible"].astype(bool))].copy()
    fixed_ours = fixed[fixed["detector_family"].isin(["timescale_transformer_v1_1", "timescale_token_mlp_control", "flat_ae_100d_control"])].copy()
    results_md = "# Timescale Tokenizer v1.1 Results\n\n"
    results_md += md_table(
        fixed[
            [
                "object_label",
    ...
    (out / "timescale_tokenizer_v1_1_results.md").write_text(results_md, encoding="utf-8")

    plot_tradeoff(df, plot_dir / "fixed_tradeoff_timescale_tokenizer_v1_1.png")
    ...
    summary = "# Frontend100 Timescale Tokenizer v1.1 Summary\n\n"
    ...
    summary += "- v1.1 changes only scoring: evaluate short-scale aware aggregations on the same trained token models.\n\n"
    ...
    summary += "- Compare score modes `mean_all / max_all / short_mean / short_max / weighted_short` under the same tokenization and training.\n"
    summary += "- Success criterion: recover attack detection materially above v1 mean-all while keeping tokenizer-side low alarm.\n"
    (out / "timescale_tokenizer_v1_1_summary.md").write_text(summary, encoding="utf-8")
    (out / "summary.md").write_text(summary, encoding="utf-8")

    cfg = {
        "stage": "frontend100_timescale_tokenizer_v1_1",
        ...
        "outputs": {
            "results": str(out / "timescale_tokenizer_v1_1_results.csv"),
            "diagnostics": str(out / "timescale_tokenizer_v1_1_diagnostics.csv"),
            "summary": str(out / "summary.md"),
            "plots": str(plot_dir),
        },
    }
    (out / "timescale_tokenizer_v1_1_config_manifest.json").write_text(json.dumps(clean(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    (out / "config.json").write_text(json.dumps(clean(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    append_map(args.run_tag)
    print(f"[done] timescale tokenizer v1.1 output: {out}", flush=True)
