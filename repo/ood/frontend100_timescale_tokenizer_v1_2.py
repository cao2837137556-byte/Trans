...
def plot_tradeoff(df: pd.DataFrame, out: Path) -> None:
    ...
    colors = {
        "timescale_transformer_v1_2": "#1f77b4",
    ...
    plt.title("Timescale tokenizer v1.2 fixed trade-off")
    plt.grid(alpha=0.25)
    plt.tight_layout()
...
def append_map(run_tag: str) -> None:
    ...
    if f"`{run_tag}`" in text:
        return
    entry = f"\n- `{run_tag}`: Frontend100 TimescaleTokenizer-v1.2 adds scale-contrast scorers on top of header-aware 5x20 regrouping; path: `runs/{run_tag}/`.\n"
    p.write_text(text.rstrip() + entry, encoding="utf-8")


def main() -> None:
    today = datetime.now().strftime("%Y-%m-%d")
    ap = argparse.ArgumentParser(description="Header-aware timescale tokenizer v1.2 for frontend100.")
    ap.add_argument("--run-tag", default=f"frontend100_timescale_tokenizer_v1_2_{today}")
    ...
    plot_dir = out / "timescale_tokenizer_v1_2_plots"
    ...
    for mode, mode_desc in score_modes:
        obj = f"timescale_transformer_{mode}"
        ...
        rows.extend(
            eval_scores(
                object_label=obj,
                detector_family="timescale_transformer_v1_2",
                score_label=mode,
                seed=args.seed,
                score_id=sid,
                score_ood=sood,
                ...
                extra={"seq_len": len(SCALES), "token_dim": token_dim, "source_mode": "computed_now", "score_mode_desc": mode_desc},
            )
        )
    diagnostics.append(
        {
            "object_label": "timescale_transformer_v1_2",
    ...
    for mode, mode_desc in score_modes:
        obj = f"timescale_token_mlp_{mode}"
    ...
    diagnostics.append(
        {
            "object_label": "timescale_token_mlp_v1_2",
    ...
    df.to_csv(out / "timescale_tokenizer_v1_2_results.csv", index=False)
    df.to_csv(out / "results.csv", index=False)
    diag.to_csv(out / "timescale_tokenizer_v1_2_diagnostics.csv", index=False)

    fixed = df[(df["policy_name"].eq("fixed_id_q99")) & (df["selection_feasible"].astype(bool))].copy()
    fixed_ours = fixed[fixed["detector_family"].isin(["timescale_transformer_v1_2", "timescale_token_mlp_control", "flat_ae_100d_control"])].copy()
    results_md = "# Timescale Tokenizer v1.2 Results\n\n"
    results_md += md_table(
        fixed[
            [
                "object_label",
    ...
    (out / "timescale_tokenizer_v1_2_results.md").write_text(results_md, encoding="utf-8")

    plot_tradeoff(df, plot_dir / "fixed_tradeoff_timescale_tokenizer_v1_2.png")
    ...
    summary = "# Frontend100 Timescale Tokenizer v1.2 Summary\n\n"
    ...
    summary += "- v1.2 keeps the same models and tokenization, but upgrades scoring to timescale-contrast variants.\n\n"
    ...
    summary += "- Core test: whether `ID-z(short) - alpha x ID-z(long)` exposes burst-like attacks better than simple short-scale pooling.\n"
    summary += "- Success criterion: improve fixed detection materially over v1.1 best while preserving tokenizer-side low alarm.\n"
    (out / "timescale_tokenizer_v1_2_summary.md").write_text(summary, encoding="utf-8")
    (out / "summary.md").write_text(summary, encoding="utf-8")

    cfg = {
        "stage": "frontend100_timescale_tokenizer_v1_2",
        ...
        "outputs": {
            "results": str(out / "timescale_tokenizer_v1_2_results.csv"),
            "diagnostics": str(out / "timescale_tokenizer_v1_2_diagnostics.csv"),
            "summary": str(out / "summary.md"),
            "plots": str(plot_dir),
        },
    }
    (out / "timescale_tokenizer_v1_2_config_manifest.json").write_text(json.dumps(clean(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    (out / "config.json").write_text(json.dumps(clean(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    append_map(args.run_tag)
    print(f"[done] timescale tokenizer v1.2 output: {out}", flush=True)
