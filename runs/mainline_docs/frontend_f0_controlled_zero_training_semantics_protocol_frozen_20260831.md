# Frontend-F0 Controlled Zero-Training Semantic Prototype Protocol (FROZEN)

- Date: 2026-08-31
- Branch: `codex/exp-mainline`
- Status: **FROZEN; NON-EXECUTABLE pending separate implementation authorization**
- Author: Codex
- User authorization consumed here: **FROZEN conversion only**
...
| other observed IP protocol, with or without decoded ports | IP endpoints, protocol, packet ordinal, finite timestamp | H2 network-protocol | finite |
...
Every observed IP protocol other than TCP or UDP is assigned to H2.  If such a
protocol exposes port fields (for example SCTP), those ports are audit-only
attributes and never enter the H2 context key.
...
portless protocol has transport ports.  More generally, any decoded port field
on a non-TCP/UDP IP protocol is audit-only and excluded from the H2 key.
...
`ZT_CONTEXT_DEGENERACY_NO_GO` may be emitted **only** when the synthetic
semantics battery fails obligations 8 or 9 (or their direct contract-test
equivalents).  Real-data context-size distributions are descriptive evidence
for the later learned-challenger design review; no observed singleton rate,
context-size quantile, or tier share can retroactively create a PASS or NO-GO
gate under this protocol.  Other real-data violations of identity, bounds,
causality, or lifecycle invariants retain their separately named terminal
states.
...
## 16. Independent-review rulings (normative)

The independent draft review at commit `2cce672` accepted all eight questions.
The following rulings are now normative:

1. **H1/H2/H3/H4 hierarchy — ACCEPT.** Context partitioning and model features
   remain structurally distinct.  C1 is incorporated: every non-TCP/UDP IP
   protocol uses H2 even when decoded ports exist, and those ports are
   audit-only attributes excluded from the context key.
2. **Literal `256 events / 300 seconds / 60 seconds idle` bounds — ACCEPT.**
   Bounds split contexts but do not create missingness, and are untunable after
   freeze.
3. **H4 base class and anti-degeneracy tests — ACCEPT with C2.** Synthetic
   tests guard forced-singleton and global-pseudo-session degeneration.
   `ZT_CONTEXT_DEGENERACY_NO_GO` is triggered only by those synthetic-battery
   failures; real-data context distributions remain descriptive evidence.
4. **Monotone timestamp surrogate — ACCEPT.** It is the sole regression policy
   and is applied identically to benign and attack events.
5. **First-seen endpoint tokens and opaque-ID boundary — ACCEPT.** Endpoint
   equality may partition contexts; endpoint identity may not enter numeric
   outputs or later tensors under this protocol.
6. **Frozen `0.90 / 0.80 / 0.80` semantic reachability gates — ACCEPT.** The
   full-universe and CE missing-subset denominators and zero-row reporting stay
   mandatory.
7. **PASS consequence — ACCEPT.** `ZT_SEMANTIC_COVERAGE_PASS` authorizes only a
   new learned-challenger DRAFT and is not any frozen F0/F1 verdict or CE
   promotion.
8. **One-candidate stop rule and mature-component-first consequence — ACCEPT.**
   No sweep, second zero-training variant, or outcome-conditioned repair is
   permitted; a PASS routes to audited mature-component sourcing under a new
   protocol.

## 17. Current authorization state

This document consumes the user's authorization to convert the independently
accepted draft into **FROZEN** form.  The protocol is now scientifically
immutable.
...
The next legal action is a separately authorized implementation and synthetic
contract-test stage.  Real PCAP execution remains independently gated after
