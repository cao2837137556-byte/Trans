...
This DRAFT authorizes no implementation, no execution, no embedding or score opening, no benign
suffix evaluation, no support-val/report/FINAL access, no PCAP decode, no download, no training,
and no HPC submission.

After independent review, a FROZEN D0 plus SHA sidecar is required. Implementation and execution
