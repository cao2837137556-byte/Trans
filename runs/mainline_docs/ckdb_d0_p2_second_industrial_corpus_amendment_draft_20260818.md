# CKDB D0-P2 second industrial/process corpus amendment — DRAFT

Date: 2026-08-18

Status: `DRAFT_FOR_KIMI_REVIEW`
...
This draft does not authorize implementation, download, form submission, HPC,
...
acceptance is automated by this protocol.
...
recovered by downloading the tar under D0-P2.

System-fault scenarios are not silently treated as benign. They remain excluded
until a later protocol defines their scientific role before opening them.

## 7. Lineage and claim ceilings

The audit must compare the PNNL capture campaign, devices, and public release
against:
...
3. missing official file inventory, license attachment, capture lineage, or
   ...
   corpus:
   `CKDB_D0_P2_PENDING_METADATA` with one or more named reason codes;
...
## 13. Questions for Kimi review

1. Accept one fixed candidate, `PNNL_ELECTRICITY_AND_GAS_IDS`, with no fallback?
2. Accept the four-condition gate for counting electricity and gas as two
   independent domains?
3. Should inability to expose a pre-open normal-member inventory remain
   `PENDING_METADATA`, or be immediate `NO_IDENTIFIABLE...`?
4. Accept excluding system-fault scenarios from the benign scope at this stage?
5. Accept the D0-P2 success state as eligibility to draft the next protocol,
   never as large-download authorization?

Until these questions are closed and a FROZEN SHA sidecar is reviewed, this
document authorizes nothing beyond discussion.
