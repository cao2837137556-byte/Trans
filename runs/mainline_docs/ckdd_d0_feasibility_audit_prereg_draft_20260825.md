# CKDD D0 — constrained attack-head retraining feasibility audit (DRAFT)

**Date:** 2026-08-25
**Status:** discussion draft; not frozen; no execution authorized
...
Without opening row values in D0, the audit must freeze the exact identity and declared coverage
of the later kill-only attack screen:

- all 51,057 already-viewed `P2 hard / M7 normal` report attacks if identity reproduction passes;
- explicit inclusion counts for `future_query`, Merlin C&C, and each of the 16 families;
...
If a later frozen CKDD head flips even one of those 51,057 rows from hard to normal, the route
closes.  Passing supplies no positive evidence and permits no iterative repair.
...
report/FINAL pre-open rejection; no optimizer step; no fitted parameter; Python 3.9 grammar and
observed runtime-API compatibility; atomic readback; engineering failure emits no scientific
verdict.
...
This DRAFT authorizes nothing.  After review it may be frozen.  Implementation, D0 execution,
training, report opening, FINAL access, downloads, and HPC all require their own authorization.
