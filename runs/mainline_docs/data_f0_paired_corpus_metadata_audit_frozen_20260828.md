# Data-F0 — Paired Cross-Device Corpus Metadata Audit (FROZEN)

- Date: 2026-08-28
- Status: **FROZEN; NON-EXECUTABLE pending independent SHA/diff terminal review and later user metadata-execution authorization**
- Freeze basis: Kimi Round 3 review `96172df`; N3 was incorporated mechanically before
  the paired-device count `N` was observed.
...
Before bulk download, report the number `N` of paired devices and the availability of
counts for independent benign sessions and attack sessions/families. The frozen minimum
and allocation are:

```text
N >= 8, else NO_IDENTIFIABLE_PAIRED_DEVICE_SPLIT
Data-E count = max(2, ceil(N / 4))
Data-T count = N - Data-E count
Data-T count >= 6, else NO_IDENTIFIABLE_PAIRED_DEVICE_SPLIT
```
...
Let `N_E = max(2, ceil(N / 4))`. The first `N_E` devices become `Data-E`; all remaining
eligible devices become `Data-T`. Data-E identities are committed before any bulk object
is opened and may never enter frontend/head training, calibration, model selection,
thresholding, or method design.

If fewer than eight paired devices exist, or the deterministic remainder contains fewer
than six Data-T devices, terminate with `NO_IDENTIFIABLE_PAIRED_DEVICE_SPLIT`. The split
may not be rescued by treating dates, files, roles, or attacks from the same device as
independent devices.
...
This FROZEN protocol authorizes no HTTP request, browser automation, login, form
submission, download, implementation, training, embedding, report/FINAL opening, or HPC
work. Independent SHA/diff terminal review is next; metadata execution requires a later
