# Frontend-F1 教师约束统一语义编码器 D0/D1 协议（DRAFT）
...
6. B 良性设备/协议覆盖表；
7. single-candidate compatibility record；
...
1. D0 是否应把 4,385 fit 攻击作为排除前锚点、并以 19-context 排除后的实际
   row/context 作为唯一训练分母？
2. label-aware 教师约束的四项 loss 结构是否足够避免蒸馏 4,986 条已知误报？
3. A shadow 的攻击继承是否应采用 target-level 零翻转硬门，还是需要一个预冻结
   的极小容差？Codex 当前主张零翻转。
4. `F1_P2_INTERFACE_NO_GO_BUT_REPRESENTATION_PASS` 的五项入口是否足够阻止
   结果驱动的新-head 尝试？
5. 51,057 viewed attack kill-only 是否应在 select PASS 后立即执行，还是留到 CE
   one-shot 前？Codex 当前主张在模型完全冻结后、one-shot 前执行。
6. hydraulic 从第一阶段验收移除但保留为后续 full-replacement 必报行，边界是否完整？
7. D0 单一候选选择是否还需更窄的词典序，或可由成熟组件兼容性 + 本地资源门
   唯一决定？

## 10. 当前授权边界

本 DRAFT 只消费用户“开始下一步、先把能力继承好”的方案起草授权。
