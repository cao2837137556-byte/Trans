# CKDE D1 — development-only benign commissioning calibration (NUMERICAL FROZEN)

**Date:** 2026-08-25
**Status:** NUMERICAL FROZEN; scientific design and literal attack-derived cap immutable; non-executable pending independent hash/diff review and separate user authorization
...
This numerical FROZEN protocol fixes the cap algorithm and inserts the separately materialized literal value.
The value was computed from the 4,385 legal fit attacks before any benign calibration score
or support-val score was opened.
...
The separately authorized Stage-P artifact freezes the following literal values:
...
The cap artifact is `runs/issue27ckde_d1_cap_materialization_v1_2026-08-25_local_r2/ckde_d1_cap.json`
with SHA-256 `4ff7b3397417b70f12c08ec928f283b14efc915bf597c486b6f4250fa92b99c8`. Its
`SHA256SUMS` has SHA-256 `d22eb39bd24004bd14473a4117ba75c1542bea22ecbafffa047c0c93e4420fe2`.
At `T_cap`, global fit-attack hard recall is `4385/4385`; global and worst eligible-family loss
are both `0.0` percentage points. The complete 2,475-candidate frontier and 29,700-row exact-family
loss table remain part of the hashed Stage-P artifact.
...
This NUMERICAL FROZEN document authorizes no benign-prefix score opening, calibration materialization,
suffix evaluation, support-val access, report access, FINAL access, training, download, or HPC
submission. Its scientific rules and literal cap are immutable.

The only possible next request is independent hash/diff review of this numerical FROZEN document
and its Stage-P artifact. After that review, Stage A still requires a new, explicit user
authorization before any eligible-device benign prefix score is opened. Stages B and C remain
separately gated under §9.
