...
Lane M: metadata-only paired-corpus reconnaissance
  ...
  M2 CICIoT2023 metadata audit only if M1 fails
...
2. **CICIoT2023** — backup metadata candidate, audited only if N-BaIoT fails.

The first candidate satisfying every acceptance condition ends the search.  If both fail,
CKDE-S external commissioning is moved to future work.  No third dataset may be searched under
this route.
...
2. `M1_CICIOT2023_METADATA_ELIGIBLE`
3. `M2_NO_IDENTIFIABLE_PAIRED_RAW_CORPUS`
...
one-shot evidence.  FINAL remains separately sealed.

## 15. Open review questions

1. Is the count-only rank rule `min(4, floor((D-1)/3))` conservative enough for 15 devices, or
   should the unique candidate be rank 3?  The choice must be literal before embedding access.
2. Are the §6 leave-one-device-out stability constants appropriately strict for a D0 screening
   gate without importing outcome selection?
3. Is `median R>=2`, 80% `R>=1` sufficient evidence that `U` represents cross-device nuisance
   rather than within-device temporal drift?
4. Should the attack protection space use frozen-head gradients as primary, with mean contrasts
   strictly diagnostic, as drafted?
5. Are residual-fraction gates 0.50/0.65 and the 25% removable-energy floor defensible, or should
   they be tightened before FROZEN?
6. Is six paired devices with three devices covering two attack families the minimum acceptable
   external mechanism-validation design?
7. Should N-BaIoT failure immediately activate CICIoT2023, or should a Kimi review gate intervene
   between candidate 1 and candidate 2 to preserve cognitive isolation?
8. Does the later external protocol need a separate zero-shot challenge-relevance gate, frozen
   now, so a corpus with no initial benign-shift problem cannot be promoted as a positive test?

## 16. Authorization boundary

This DRAFT authorizes nothing.  After review, Codex may generate a FROZEN protocol and SHA sidecar
