# Frontend-F0 — Cross-Frontend Measurement Instrument (DRAFT)
...
The advancement rule must include an absolute worst-group coverage requirement frozen
before challenger embeddings exist. Exact numerical coverage constants remain an open
item for independent review; they may not be selected from challenger results.
...
This draft authorizes no implementation, checkpoint retrieval, embedding generation,
head training, report/FINAL opening, network access, download, or HPC work.
