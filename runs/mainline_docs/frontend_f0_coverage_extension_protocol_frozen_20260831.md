# Frontend-F0 Coverage Extension Protocol (FROZEN)

- Date: 2026-08-31
- Status: **FROZEN; ROUTE-LEVEL; NON-EXECUTABLE**
- Author: Codex (primary implementation/design role)
- Governing review chain:
  - Kimi CE route ruling: `539b313`
  - Kimi draft review: `d76915a`
  - M1 denominator clarification: `6accfd1`
  - Kimi M1 narrow ruling: `7f2c567`
...
This protocol does not nominate a challenger, authorize a parser, retrieve a
...
The incumbent is frozen as candidate `E3`, probe `P2`, with threshold
`0.065159872174263`. Its immutable identity is:

| Artifact | SHA-256 |
|---|---|
| `runs/.issue27ckda_d1_representation_probe_v1_2026-08-14_localwin_cpu.stage/ckda_d1_select_scores.csv.gz` | `bc34268eea2c4545a425cba8adf641a214f75dae5435858a86ee0bb3aabe3419` |
...
The threshold frontier identity is
`aa36fc5f096fcfbfc1175a14f760b9a48e2be2a2e01a561f4598ef2a8ed41958`.
The exact gate is:
...
### 4.3 Frozen CE-5 baseline and missing-score semantics

The CE-5 eligible incumbent baseline is reconstructed mechanically as:
...
It contains exactly **4,812** rows:

| Role | Rows |
|---|---:|
| `aux_normal_select` | 3,518 |
| `aux_select` | 1,294 |
| **Eligible denominator** | **4,812** |

The 381 old-missing `ood_val` rows and 49 old-missing `id_calib` rows are
`phase=fit` and are excluded. The withdrawn 5,242 count must not enter any
CE selection or utility verdict.

For every one of the 4,812 eligible rows, the incumbent P2 score is exactly
equal to the threshold literal `0.065159872174263`. The frozen threshold
application is `score >= threshold`, so all **4,812/4,812** are incumbent-hard.
Thus `H_old = 4,812` is a consequence of the incumbent's fail-closed
missing-score pinning convention, not learned P2 separation on these rows.
A CE hard-count reduction therefore means that the challenger emits a finite,
below-threshold decision for a row the incumbent pinned at its threshold.

The frozen material-gain boundary is:

```text
max(300, ceil(0.10 * 4,812)) = 482
```

The P2 baseline also makes all 69/69 select attacks hard and all 23/23
old-missing `support_val` attacks hard. CE-5 condition 5 therefore subsumes
condition 4 on this frozen baseline. Condition 4 remains as an explicit,
redundant sparse-denominator guard so the 23-row missing-attack subset cannot
be hidden inside the larger select-attack denominator.

### 4.4 Challenger branch
...
This stage requires `F0_ENCODER_ONLY_PASS` plus frozen count, identity, and
copy artifacts. It does **not** require `F1_FRONTEND_CHALLENGE_PASS`, because
it opens no challenger representation values, scores, or performance evidence.
...
This stage requires `F1_FRONTEND_CHALLENGE_PASS` and separate user
authorization. Only after CE-0 through CE-3 PASS and those two prerequisites
may the frozen challenger head emit scores on legal fit/select rows. The CE
result is shadow-only: it cannot alter the formal alarm stream.
...
8. total old-missing benign-select hard-count reduction is at least **482**,
   the frozen value of `max(300, ceil(0.10 * H_old))` for
   `H_old = 4,812` on exactly the eligible old-missing benign-select rows;
...
The independent-review principle that a future `H_old < 300` would make the
absolute 300-row material-gain boundary impossible is accepted. It is not a
runtime branch in this frozen experiment: the pinned incumbent baseline fixes
`H_old = 4,812` and the literal boundary at 482.
...
No performance threshold or final promotion state is inferred by this
protocol; those values must be frozen in CE-6 before report access.
...
18. the frozen material-gain boundary is exactly 482 rows;
19. the eligible old-missing benign-select denominator is exactly 4,812 and
    contains no fit-role row;
20. all 4,812 incumbent missing scores equal the threshold literal and become
    hard only through the frozen `>=` fail-closed convention;
21. CE-2 rejects execution without `F0_ENCODER_ONLY_PASS` and frozen
    count/identity/copy artifacts, while remaining independent of F1;
22. CE-4 rejects execution without `F1_FRONTEND_CHALLENGE_PASS` and separate
    user authorization;
23. viewed/report attack outcomes can only kill an already frozen candidate;
24. a CE failure cannot activate full replacement unless its literal state is
    whitelisted in §10.2;
25. Python 3.9 syntax and runtime API compatibility pass; and
26. every durable output survives readback and SHA verification.
...
## 13. Independent-review rulings (normative)

All six questions in the draft review are closed as follows:

1. **Q1 accepted.** The 23/23 missing-`support_val` attack requirement is a
   kill-only development guard. It is non-promotional, too sparse for a
   family-level claim, and must be reported with literal per-family counts.
2. **Q2 accepted after M1.** Conditions 6–8 make material benign gain
   mechanical on the frozen 4,812-row select denominator. Aggregation is by
   benign device only; device identity never enters routing or model features.
3. **Q3 accepted and materialized.** The absolute floor remains scientifically
   justified. For this frozen baseline `H_old=4,812`, so the only executable
   boundary is the literal value 482; no small-`H_old` scaling branch exists.
4. **Q4 accepted.** Later one-shot confirmation must preserve every
   incumbent-hard attack at target level. A global recall tolerance may not
   hide changed families or individual old-hard targets.
5. **Q5 accepted.** The §10.2 whitelist is sufficiently narrow:
   full-replacement discussion can open only after safe, material CE gain is
   established and the remaining whole-system failure is specifically on
   incumbent-finite targets.
6. **Q6 accepted with explicit stage mapping.** CE-2 requires
   `F0_ENCODER_ONLY_PASS` plus frozen count/identity/copy artifacts and does
   not require F1. CE-4 requires `F1_FRONTEND_CHALLENGE_PASS` plus separate
   user authorization.

The M1 clarification and narrow ruling additionally freeze the incumbent as
E3/P2, the five artifact identities and threshold identity in §4.2, the
4,812-row denominator, `H_old=4,812`, the 482-row gate, the condition 4/5
overlap, and the missing-score pinning interpretation in §4.3.

## 14. Authorization boundary

This FROZEN protocol authorizes no implementation, candidate retrieval,
network access, checkpoint download, corpus download, PCAP decode,
