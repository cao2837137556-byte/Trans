# Frontend-F0 Step-0b Causal Re-decode Attribution Audit — FROZEN Preregistration

- Date: 2026-08-29
- Status: **FROZEN — NOT EXECUTABLE; IMPLEMENTATION REQUIRES USER AUTHORIZATION**
...
whole-archive bytes and SHA-256 are also mandatory and are recorded before any decode.
The already published archive identity plus the ZIP central-directory member name,
uncompressed bytes, and CRC32 remain mandatory as independent provenance and
decode-integrity checks. An archive may not use its published identity or CRC32 values
as a substitute for the SHA-256 identity of the exact local container bytes.
...
8. all local packet containers exist and match whole-file bytes and SHA-256; the Gotham
   archive additionally matches its published identity and every allowlisted member's
   central-directory uncompressed bytes and CRC32; and
...
This equivalence gate validates the reconstructed OR of the four predicates—that is,
the frozen `missing` decision. It does not by itself validate which individual
predicate is true on an overlapping missing row. Individual-predicate fidelity rests on
the literal frozen semantics in §4, tests 9–20, preservation of the pinned execution
order, and verbatim reporting of parser defaults. The result report and verdict may not
cite R3 alone as proof of per-predicate attribution correctness.
...
state vocabulary and is presence-based. A mechanism class is present iff its primitive
predicate is true for at least one missing target, whether that predicate is primary or
secondary under the descriptive precedence:
...
2. Otherwise, if exactly one non-configuration mechanism class is present by the
   any-true rule and no configuration-only cause is present, return
...
For every mechanism class, the verdict carries an `any_true_missing_target_count`
computed from the union over the corresponding primitive boolean. It separately carries
the inherited `primary_reason` distribution. Primary-reason counts may not determine
mechanism presence. A missing target is recoverable only after every true predicate on
that target is addressed.

Because the classification uses any-true non-zero presence rather than a post-hoc
percentage, the observed distribution cannot change the rule. Exact percentages and
the primary-reason distribution are descriptive.
...
2. packet identity attachment is completed before any packet open, and a missing or
   mismatched Gotham whole-archive SHA-256 fails before member decoding even when
   published identity and member CRC32/size match;
...
19. multiple true predicates are retained while precedence is deterministic, and every
    corresponding mechanism class receives an any-true presence/count independent of
    the single descriptive primary reason;
...
## 11. Normative review rulings

The independent draft review at commit `18d5c88` is incorporated as follows:

1. Gotham whole-archive SHA-256 is mandatory in R0; published identity and per-member
   CRC32/size remain additional provenance and decode-integrity checks.
2. The formal IP-key, append-before-target-gate, timestamp-poison, equality, causal
   current-and-later, and tail-release semantics in §4 are confirmed against the pinned
   implementation.
3. Exact 25,467/25,467 equivalence is sufficient to validate the reconstructed OR and
   frozen missing decision without recomputing embeddings, subject to the explicit
   individual-predicate validation limitation in R3.
4. Route naming is retained, but mechanism presence is determined only from the union
   of all any-true primitive predicates; `primary_reason` is descriptive.
5. The packet identity attachment remains an implementation artifact under the frozen
   schema and nine fail-closed R0 assertions and must accompany implementation review.
6. Member-atomic checkpoints and the local offline resource gates are accepted.

## 12. Authorization boundary

This FROZEN protocol authorizes nothing by itself. The next legal action is independent
SHA/diff terminal verification of this document and its SHA-256 sidecar.

After that verification, implementation and the synthetic contract-test battery still
require explicit user authorization. Packet opening and the real causal re-decode require
a later, separate user execution authorization after independent implementation review.
Training, embedding generation, checkpoint inference, network retrieval, report access,
FINAL access, HPC submission, and any performance claim remain unauthorized.
