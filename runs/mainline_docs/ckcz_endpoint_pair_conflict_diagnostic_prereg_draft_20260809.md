# CKCZ：Endpoint-Pair Conflict Persistence 只读诊断预注册（DRAFT）

> 状态：**DRAFT — NOT FROZEN — NOT AUTHORIZED FOR IMPLEMENTATION OR HPC**
>
> 日期：2026-08-09。Codex 负责起草、实现与后续打包；Kimi/GPT 负责独立方案审查；
> 用户负责冻结与 HPC 提交授权。审查完成前不得写正式 exporter/diagnostic、构建 bundle
> 或提交作业。
...
正式 freeze 前必须对 auxiliary cache 做与 Gotham 相同的在线存在性检查。若缺失，CKCZ
中止并报告，不重解码、不静默降为三池。
...
正式 freeze 必须附带两个显式 allowlist CSV：Gotham 24 source、auxiliary 31 source，
并分别生成 SHA-256 侧车。Exporter 的执行顺序固定为：
...
- 16 attack family 全表、相对 C1 差值、worst family；
...
- future recall `>=84.83%`；
- rows>=15 的主要 family 相对 C1 不恶化超过 2pp；
- support_val 69/69；
- 四池 OOD macro `<=30.27%`；
- review=0。
...
1. 是否接受 auxiliary cache 是四池完整 Oracle 的必要输入；
2. 是否接受 24 Gotham source / 317,523 rows + 31 auxiliary source / 18,600 rows 合同；
3. 是否接受四个且仅四个 scalar，以及 current-conflict 必须为真的 veto 形式；
4. 是否接受“全部 oracle frontier 失败才判死；任一可行只授权 D1-Legal”的非晋升规则；
5. 是否发现 FINAL allowlist、protocol slice、hydraulic missing-state 或 auxiliary UID 合同缺口；
6. 是否需要删除某个统计量以降低 VIEWED feature-search 风险。

审查意见只写入新 review 文档；不得静默修改本 DRAFT。审查通过后由 Codex 生成新的
FROZEN 文件与 SHA-256 侧车，用户明确授权后才开始实现。
