...
RUN_ROOT="$BASE/runs/issue27ckbu_unified_process_rescue_formal_v1_2026-07-23_seed27_${PARTITION}_${JOB_ID}"
ARCHIVE="$BASE/runs/issue27ckbu_seed27_${PARTITION}_${JOB_ID}_pullback.tar.gz"
test -d "$RUN_ROOT" || { echo "missing CKBU run root: $RUN_ROOT" >&2; exit 2; }
...
if int(review.review_count.sum()) or float(review.review_rate.max()) != 0.0: raise SystemExit("review is not zero")
env=json.loads((root/"ckbu_environment.json").read_text())
if str(env.get("slurm_job_id")) != job or env.get("slurm_partition") != partition: raise SystemExit("job identity drift")
print(json.dumps({"status":"CKBU_RESULT_VALID","decision":decision["decision"],"partition":partition,"job_id":job},indent=2))
PY

rm -f "$ARCHIVE" "$ARCHIVE.sha256"
tar -C "$RUN_ROOT" -czf "$ARCHIVE" \
  --exclude='gotham_causal_cache' --exclude='auxiliary_causal_cache' --exclude='source_runtime' .
cd "$BASE/runs"
sha256sum "$(basename "$ARCHIVE")" > "$(basename "$ARCHIVE").sha256"
sha256sum -c "$(basename "$ARCHIVE").sha256"
echo "CKBU_PULLBACK_ARCHIVE=$ARCHIVE"
