#!/bin/bash
# CKBE r3 remote preparation.  This script does not submit Slurm work.
# It atomically prepares the complete offline PyG runtime and frozen event plan.
set -euo pipefail
...
BUNDLE="${1:-$HOME/work/_upload_issue27ckbe_r3}"

test -f "$BUNDLE/repo/ood/issue27ckbe_tgn_fullsupport_event_cache_v1.py" || {
  echo "missing CKBE r3 bundle at $BUNDLE" >&2
  exit 2
}
