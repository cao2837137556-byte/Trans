...
show_one() {
  ...
  run_root="$BASE/runs/issue27ckbq_causal_minirocket_consensus_v1_2026-07-17_seed27_${partition}_${job_id}"
  ...
  if test -s "$run_root/ckbq_single_seed_go_no_go.json"; then
    echo "--- scientific decision ---"
    cat "$run_root/ckbq_single_seed_go_no_go.json"
  elif test -s "$run_root/job_failure.txt"; then
    echo "--- failure marker ---"
    cat "$run_root/job_failure.txt"
    echo "--- stderr ---"
    tail -n 80 "$BASE/runs/issue27ckbq_${partition}_${job_id}.err" 2>/dev/null || true
  else
    echo "--- latest stdout ---"
    tail -n 40 "$BASE/runs/issue27ckbq_${partition}_${job_id}.out" 2>/dev/null || true
    echo "--- latest stderr ---"
    tail -n 40 "$BASE/runs/issue27ckbq_${partition}_${job_id}.err" 2>/dev/null || true
  fi
}
